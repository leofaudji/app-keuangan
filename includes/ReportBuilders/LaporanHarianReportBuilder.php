...
class LaporanHarianReportBuilder implements ReportBuilderInterface
{
    ...
    private function render(array $data): void
    {
        ...
        $this->pdf->Cell(30, 7, 'ID/Ref', 1, 0, 'C', true); $this->pdf->Cell(115, 7, 'Keterangan', 1, 0, 'C', true); $this->pdf->Cell(60, 7, 'Akun Terkait', 1, 0, 'C', true); $this->pdf->Cell(35, 7, 'Pemasukan', 1, 0, 'C', true); $this->pdf->Cell(35, 7, 'Pengeluaran', 1, 1, 'C', true);
        ...
        if (empty($transaksi)) { $this->pdf->Cell(275, 7, 'Tidak ada transaksi pada tanggal ini.', 1, 1, 'C'); } 
            ...
            foreach ($transaksi as $tx) {
                ...
                $this->pdf->Cell(30, 6, $idDisplay, 1, 0); $this->pdf->Cell(115, 6, $tx['keterangan'], 1, 0); $this->pdf->Cell(60, 6, $tx['akun_terkait'] ?? '', 1, 0); $this->pdf->Cell(35, 6, $tx['pemasukan'] > 0 ? format_currency_pdf($tx['pemasukan']) : '-', 1, 0, 'R'); $this->pdf->Cell(35, 6, $tx['pengeluaran'] > 0 ? format_currency_pdf($tx['pengeluaran']) : '-', 1, 1, 'R');
            }
        }

        $this->pdf->SetFont('Helvetica', 'B', 10);
        $this->pdf->Cell(205, 7, 'TOTAL', 1, 0, 'R', true); $this->pdf->Cell(35, 7, format_currency_pdf($total_pemasukan), 1, 0, 'R', true); $this->pdf->Cell(35, 7, format_currency_pdf($total_pengeluaran), 1, 1, 'R', true);
        $this->pdf->Ln(5);
        $this->pdf->SetFont('Helvetica', 'B', 10);
